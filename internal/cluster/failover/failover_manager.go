...
func (fm *FailoverManager) handleRoleTransition(oldRole, newRole NodeRole) {
	...
	case RoleReplica:
		if err := fm.transitionToReplica(); err != nil {
			fm.logger.Error("Failed to transition to replica", "error", err)
		}
	}
}
...
// transitionToReplica transitions the node to replica role
func (fm *FailoverManager) transitionToReplica() error {
	fm.logger.Info("Starting transition to replica")

	// Stop any active replication first
	fm.replicationManager.StopReplica()

	// Find the current leader/primary to replicate from
	leader := fm.raftNode.GetLeader()
	if leader == nil {
		return fmt.Errorf("no leader found to replicate from")
	}

	leaderAddr := string(*leader)

	// Start replication from the leader
	if err := fm.replicationManager.StartReplica(leaderAddr); err != nil {
		return fmt.Errorf("failed to start replication: %w", err)
	}

	fm.logger.Info("Successfully transitioned to replica", "leader", leaderAddr)
	return nil
}
