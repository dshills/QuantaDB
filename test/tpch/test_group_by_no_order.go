...
func main() {
	...
	fmt.Println("=== GROUP BY Without ORDER BY Test ===")

	// Test the same query but without ORDER BY to isolate the issue
	fmt.Println("Testing SELECT with GROUP BY (no ORDER BY):")
	fmt.Println("Query: SELECT c_mktsegment, COUNT(*) as cnt FROM customer GROUP BY c_mktsegment")
